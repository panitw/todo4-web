...
import { restoreTask, type Task, type TaskListMeta } from '@/lib/api/tasks';
...
export function useRestoreTask() {
  ...
  return useMutation({
    ...
    onMutate: async (id) => {
      ...
      // Optimistically mark as open in all caches
      queryClient.setQueriesData<{ data: Task[]; meta: TaskListMeta }>(
        ...
        (old) => {
          ...
          return {
            ...old,
            data: old.data.map((t) =>
              t.id === id ? { ...t, status: 'open' as const, deletedAt: null } : t,
            ),
          };
        },
      );
